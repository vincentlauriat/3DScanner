...
| Domain | `RoomScanner/Domain` | `ScanInput` (instantané neutre), types pivots `FloorPlan` / `House` / `Story`, `FloorPlanBuilder` (ScanInput → projection 2D), `WallGeometry` (boîtes de murs / panneaux partagés par le visualiseur et l'export de maillage), `Measurements`, `RoomNaming` ; v2 : `StructureInput` (scans par pièce en repère monde partagé + surfaces fusionnées), `StoryDetector` (niveaux par hauteur de sol), `HouseBuilder` (StructureInput → `House`) | Foundation, simd uniquement |
...
| MacUI | `RoomScanner/MacUI` (cible macOS uniquement) | `MacRootView` (`NavigationSplitView`, recherche, menu contextuel, glisser du paquet), `MacAppState` (sélection + actions de menu via `focusedSceneValue`), `MacMenuCommands` (⌘O importer, ⌘E exporter + sous-menu par format, ⌘P imprimer, ⇧⌘R Révéler dans le Finder, ouvrir le dossier), `PrintController` (`NSPrintOperation` sur le PDF vectoriel), `DragExportProvider` (`NSItemProvider.registerFileRepresentation`, généré à la demande), `OpenWithMenu` (`NSWorkspace.urlsForApplications(toOpen:)`), `MacEmptyStateView`, `UpdaterController` (Sparkle `SPUStandardUpdaterController`, démarré seulement si `SUPublicEDKey` est présente) | UI, Export, AppKit, Sparkle |
| UI | `RoomScanner/UI` | Écrans SwiftUI : liste (confirmation de suppression, réglages), détail (Plan 2D / 3D / Mesures), feuille d'export, `CloudStatusBadge`, écrans d'erreur, `SettingsView` (bascule iCloud, dossier, mises à jour sur Mac, à propos) | App, Domain, Export, Storage, QuickLook |
...
- `HouseMeasurements` — surface de la maison = **union** des pièces de chaque niveau (les niveaux sont additionnés puisqu'ils se superposent en plan), nombre de pièces et de niveaux. Deux pièces mitoyennes se recouvrent légèrement (épaisseur des murs, contours de scan imprécis) : sommer leurs aires double-compte ce recouvrement — mesuré à +2,14 m² sur la première maison réelle (D27). `Polygon2D.unionArea` applique l'inclusion–exclusion et ne découpe jamais que par un triangle, seul cas où Sutherland–Hodgman est valide.
- `RoomNaming` — une pièce est nommée depuis **toutes** les sections RoomPlan de sa capture (« Salle à manger / Cuisine ») : un `CapturedRoom` peut en porter plusieurs. `.unidentified` est écarté dès qu'une autre section existe ; les doublons sont numérotés (« Chambre 2 »).
- **Aucun dédoublonnage au niveau maison (D26)** — sur la première vraie `CapturedStructure`, aucun identifiant de mur n'est partagé entre deux pièces : `StructureBuilder` écarte 7 murs de pièce et en synthétise 3 (52 → 48), et fusionne les 3 sols de pièce en 1. Portes, fenêtres, ouvertures et objets passent inchangés. Le mur mitoyen est donc une reconstruction, pas un doublon repérable — `StructureInput.mergedWalls` / `mergedFloors` portent les surfaces fusionnées d'Apple pour la géométrie de la maison ; les `FloorPlan` par pièce restent la source du par-pièce.
...
| ZIP | `ArchiveExporter` : tous les exports + `README.txt` localisé dans `<Pièce>/`, compressés via `NSFileCoordinator(.forUploading)` (sans dépendance, iOS + macOS) | — |
...
<racine>/Documents/                    racine = conteneur iCloud (affiché « iCloud Drive / 3D Scanner ») ou repli local
  ...
  Houses/<uuid>.housescan/             v2 — un paquet par maison (UTType fr.vincentlauriat.roomscanner.house)
    meta.json             HouseRecord (nom, nombre de pièces/niveaux, surface)
    house.json            House (niveaux → FloorPlans, repère monde partagé) — recalculé depuis structure.json quand il existe
    structure.json        StructureInput (neutre : ScanInputs par pièce + surfaces fusionnées)
    structure-apple.json  CapturedStructure (lisible sur iOS seulement ; source de vérité)
    rooms/<uuid>.roomscan paquets de pièces imbriqués (non listés comme pièces libres)
    house.usdz            export paramétrique de la structure entière
    thumbnail.png         aperçu du rez-de-chaussée
```
...
- Flux de scan v2 (fait, validation device en attente) : `ScanCoordinator(mode: .house)` conserve une seule `ARSession` entre les pièces (`RoomCaptureView(frame:arSession:)`, `stop(pauseARSession: false)`), accumule les `CapturedRoom`, puis `StructureBuilder(options: [.beautifyObjects]).capturedStructure(from:)` → `CapturedStructure` → `CapturedStructureAdapter` → `StructureInput` → `HouseBuilder` → `HousePackage.assemble` → `.housescan`. `ScanView` pilote avec « Pièce terminée » / « Pièce suivante » / « Terminer la maison » ; la barre d'outils propose « Scanner une pièce / une maison » (`ScanMode`).
- En aval : `PlanRenderer`, `PlanSceneBuilder` et tous les exporteurs itèrent sur `house.allRooms`. Une maison produit une page PDF par niveau (`PlanRenderer.pages(of:)`, pages PNG empilées) et une teinte de sol pastel par pièce (`PlanRenderer.roomPalette`, plan 2D et texture du sol 3D) ; `HouseDetailView` filtre plan, visualiseur et mesures par niveau.
- Stockage v2 (fait) : paquets `Documents/Houses/<uuid>.housescan/` (`HousePackage`, `HouseRecord`) ; `RoomStore` expose `houseRecords`, `saveHouse`, `house(for:)`, `importAny` ; `LibraryItem` (pièce | maison) pilote la bibliothèque à deux sections iOS et la barre latérale Mac ; `ExportSubject` (pièce | maison) est l'entrée unique d'`ExportService`, `ExportSheet`, `DragExportProvider` et du README d'archive. Reste : capture multi-pièces sur iPhone (`CapturedStructureAdapter`), dédoublonnage du mur mitoyen, résolution de conflits `.housescan`.
